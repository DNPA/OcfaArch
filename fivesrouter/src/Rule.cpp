...
std::ostream& router::operator<<(std::ostream& out, Rule& rule){
	out << *(rule._Expression) << " ";

       switch(rule.getFinalAction()->getType()){
       case FinalAction::FACT_COMMIT:
               out << "ACT_COMMIT; " << dynamic_cast<FinalActionCommit *>(rule.getFinalAction())->getValue();   break;
       case FinalAction::FACT_FORWARD:
               out << "ACT_FORWARD; "<< dynamic_cast<FinalActionForward *>(rule.getFinalAction())->getValue(); break;
       case FinalAction::FACT_JUMP:
               out << "ACT_JUMP; " << dynamic_cast<FinalActionJump *>(rule.getFinalAction())->getValue(); break;
/*     case FACT_SUSPEND:
       case FACT_RETURN:
       case FACT_DONOTHING:
       default:
       neither implemented nor used in this kind of rulelist 
*/
       }
       out << ";";
       return out;
