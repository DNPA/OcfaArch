...
Router::Router()
  ...
  mDoIntegerVals = (OcfaConfig::Instance()->getValue("routeonint") != "false");
  ...
  mDoExtendedEvidence = (OcfaConfig::Instance()->getValue("extendedevidence") != "false");
} //constructor
...
void Router::processEvidence() {
 ...
 jobiterator= extendableEvidence->getJobIterator();
  ...
  do { /*while jobiterator->next() */
    ...
    if(!jobiterator->isDone()) {  //for all jobs not done (new or processed)
  ...
  FinalAction *fa= mRuleList->parsePacketVector(packetcontainer, "default");
  logEvidence(LOG_DEBUG,string("routing as a result of: PACKET ") +mRuleList->getMatchPacketString("default")+ " , RULE " + mRuleList->getMatchRuleString("default"));
  PerformRouting(fa);
