...
Lexer::Lexer(const std::string &filename) throw(ocfa::misc::OcfaException): OcfaObject("lexer", "core"), token(0), lexeme() {
	file.open(filename.c_str(), std::ifstream::in);
	if (!file){
		throw ocfa::misc::OcfaException("Cannot open file", this);
	}
	linenumber = 1;
	next();
}
